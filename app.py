...
def render_cadastral_pendencies(
    df: pd.DataFrame,
):
    ...
    def render_pending_table(
        ...
        for internal_name, label in preferred_columns:
            if internal_name in filtered_df.columns:
                available.append(
                    (
                        internal_name,
                        label,
        ...
        if not available:
            ...
            display = filtered_df[
                [
                    internal_name
                    for internal_name, _ in available
                ]
            ].copy()
            ...
            display = display.rename(
                columns={
                    internal_name: label
                    for internal_name, label in available
                }
            )

        grid = GridOptionsBuilder.from_dataframe(
            display
        )
    ...
    if name_column and "nome" not in df_work.columns:
        df_work["nome"] = df_work[name_column]

    if cpf_column and "cpf" not in df_work.columns:
        df_work["cpf"] = df_work[cpf_column]

    if address_column and "endereco" not in df_work.columns:
        df_work["endereco"] = df_work[address_column]

    if (
        cadastro_column
        and "cadastroatualizado"
        not in df_work.columns
    ):
        df_work["cadastroatualizado"] = (
            df_work[cadastro_column]
        )

    if "cpf" in df_work.columns:
        sem_cpf = missing_value(
            df_work["cpf"]
        )
    else:
        sem_cpf = pd.Series(
    ...
    if "endereco" in df_work.columns:
        sem_endereco = missing_value(
            df_work["endereco"]
        )
    else:
        sem_endereco = pd.Series(
            False,
            index=df_work.index,
        )

    if "cadastroatualizado" in df_work.columns:
        ...
        cadastro_desatualizado = pd.Series(
            False,
            index=df_work.index,
        )
    ...
    st.caption(
        ...
        f"{int(cadastro_desatualizado.sum())}"
    )

    tab_cpf, tab_endereco, tab_cadastro = st.tabs(
        [
            "Sem CPF",
            "Sem endereço",
            "Cadastro desatualizado",
    ...
    with tab_endereco:
        render_pending_table(
            df_work.loc[sem_endereco],
            "grid_pendencias_sem_endereco",
        )

    with tab_cadastro:
        render_pending_table(
            df_work.loc[cadastro_desatualizado],
...
def render_nominal(
    ...
    cols.extend(
        [
            column
            for column in indicator_cols_map.get(
                spec.code,
                [],
            )
            if column in df.columns
    ...
    boa_praticas_df = build_good_practices_df(
        df,
        spec,
    )
    ...
    tab_labels = [
        TXT["lista_geral"],
        *[
            TXT["pendencia"].format(
                l=letra,
                label=TAB_SHORT_LABELS
                .get(spec.code, {})
                .get(letra, ""),
            )
            for letra in letras
    ...
    selected_tab = st.radio(
        ...
        options=tab_labels,
        horizontal=True,
        key=f"lista_nominal_{spec.code}",
    )
    ...
    def render_grid(
        ...
        if "Endereço" in table_df.columns:
            grid.configure_column(
                "Endereço",
                width=360,
                minWidth=280,
        ...
        st.caption(
            TXT["total_pacientes_exibidos"].format(
                n=len(table_df)
            )
        )
    ...
    if selected_tab == tab_labels[0]:
        render_grid(
            df_display,
            f"grid_{spec.code}_geral",
        )

        csv_bytes = df_display.to_csv(
            index=False
        ).encode("utf-8-sig")
        ...
        selected_index = tab_labels.index(
            selected_tab
        )

        letra = letras[selected_index - 1]

        boa_pratica_col = None

        if spec.code == "C7":
            candidate = f"c7_{letra.lower()}_ok"

            if candidate in df.columns:
                boa_pratica_col = candidate
        else:
            candidates = [
                column
                for column in spec.weights
                if re.search(
                    rf"_{letra.lower()}_",
                    column,
                )
            ...
            if candidates:
                boa_pratica_col = candidates[0]

        if boa_pratica_col is None:
            filtered_df = df.iloc[0:0].copy()
        else:
            filtered_df = df[
                ~to_bool(df[boa_pratica_col])
            ].copy()

        if (
            spec.code == "C7"
            and letra in {"A", "B", "C", "D"}
            and "idade" in filtered_df.columns
        ):
            age_rules = {
                "A": (25, 64),
                "B": (9, 14),
            ...
            min_age, max_age = age_rules[letra]

            filtered_df = filtered_df[
                filtered_df["idade"].between(
                    min_age,
                    max_age,
                    inclusive="both",
                )
        ...
        csv_bytes = filtered_display.to_csv(
            index=False
        ).encode("utf-8-sig")
