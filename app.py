...
def exibir_cabecalho_analise(linha_cuidado: str, origem: str, equipes_em_analise=None):
    selo = "Detectado automaticamente" if origem == "automática" else "Definido manualmente"
    texto_equipes = "Todas as equipes presentes no relatório"
    if equipes_em_analise:
        equipes_validas = [str(e).strip() for e in equipes_em_analise if pd.notna(e) and str(e).strip()]
        if equipes_validas:
            if len(equipes_validas) <= 5:
                texto_equipes = ", ".join(equipes_validas)
            else:
                texto_equipes = f"{len(equipes_validas)} equipes selecionadas: " + ", ".join(equipes_validas[:5]) + "..."
    st.markdown(f"""
    <div style='background:#f8fafc;border:1px solid #e5e7eb;border-radius:16px;padding:16px 18px;margin:8px 0 14px 0;'>
        ...
        <div style='font-size:0.95rem;color:#475569;margin-bottom:8px;'>{selo}</div>
        <div style='font-size:0.8rem;color:#64748b;font-weight:600;letter-spacing:.02em;text-transform:uppercase;margin-bottom:4px;'>Equipes em análise</div>
        <div style='font-size:0.95rem;color:#334155;'>{texto_equipes}</div>
    </div>
    """, unsafe_allow_html=True)
...
def carregar_planilha(uploaded_file):
    ...
    if nome.endswith('.xls'):
        try:
            ...
            try:
                buffer.seek(0)
                tabelas = pd.read_html(buffer)
                if tabelas:
                    st.info("O arquivo .xls parece ter sido exportado em formato HTML/legado. A leitura foi feita com fallback automático.")
                    return tabelas[0]
            except Exception:
                pass
            st.error("Não foi possível ler este arquivo .xls no formato original. Isso costuma acontecer quando o sistema exporta um arquivo legado incompatível. Se possível, abra e salve como .xlsx antes do upload.")
            raise ValueError("Falha ao ler arquivo .xls. Tente converter para .xlsx.")
    ...
    try:
        ...
        buffer.seek(0)
        return pd.read_csv(buffer, encoding='latin1', sep=None, engine='python')
...
def render_diabetes(df, origem_cabecalho="automática"):
    df, m = preparar_diabetes(df)
    st.sidebar.header("Filtros")
    filtrado = aplicar_filtros_base(df, m["equipe"], m["micro"], "Prioridade")
    equipes_filtradas = sorted(filtrado[m["equipe"]].dropna().astype(str).str.strip().unique().tolist()) if m["equipe"] in filtrado.columns else None
    exibir_cabecalho_analise("Diabetes", origem_cabecalho, equipes_filtradas)
    total = len(filtrado)
...
def render_hipertensao(df, origem_cabecalho="automática"):
    df, m = preparar_hipertensao(df)
    st.sidebar.header("Filtros")
    filtrado = aplicar_filtros_base(df, m["equipe"], m["micro"], "Prioridade")
    equipes_filtradas = sorted(filtrado[m["equipe"]].dropna().astype(str).str.strip().unique().tolist()) if m["equipe"] in filtrado.columns else None
    exibir_cabecalho_analise("Hipertensão", origem_cabecalho, equipes_filtradas)
    total = len(filtrado)
    pct = lambda col: f"{((filtrado[col] == 'S').mean() * 100 if total else 0):.1f}%"
    pct_visita = f"{(((filtrado['Visitas Normalizadas'] > 0).mean() * 100) if total else 0):.1f}%"
...
if arquivo is None:
    ...
    try:
        ...
        if secao == "Hipertensão":
            render_hipertensao(df, origem)
        else:
            render_diabetes(df, origem)
    except Exception as e:
